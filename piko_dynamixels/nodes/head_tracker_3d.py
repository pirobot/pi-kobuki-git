...
"""
    head_tracker_wd.py - Version 2.0 2013-08-23
...
class HeadTracker():
    def __init__(self):
        ...
        self.search_target_timeout = rospy.get_param('~search_target_timeout', 5)
        ...
        # Use a flag to turn on or off the tracking
        self.active = True
        
        # Subscribe to the /head_tracker/active topic to listen for on/off messages
        rospy.Subscriber('/head_tracker/active', Bool, self.toggle_active)
...
#                         tilt_speed /= 1.1
                        ...
                        # Check if the target has been lost for more than a second
                        if self.target_lost_time > 1.5:
    ...
    def update_joint_speeds(self, msg):  
        try:
            ...
            if msg == PointStamped() or not self.active:
                return
    ...
    def update_joint_positions(self, msg):
        try:
            ...
            # Some publishers will continue to publish an empty message even when there is no 
    ...
    def toggle_active(self, msg):
        rospy.loginfo("GOT ACTIVE COMMAND: " + str(msg.data))
