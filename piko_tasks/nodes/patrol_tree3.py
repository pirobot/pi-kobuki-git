...
class BlackBoard():
    def __init__(self, name, fake, debug): 
        self.debug= debug           
        if fake:
            ...
            self.robot_charging_complete = None
            self.on_dock = False
...
class Patrol():
    def __init__(self):
        ...
        if self.fake:
            ...
            #STAY_HEALTHY.add_child(CHECK_CHARGING)
...
#             self.blackboard.on_dock = False
    ...
    def check_on_dock_cb(self):
        ...
        if self.fake:
            return self.blackboard.robot_charging
        else:
            if self.blackboard.robot_charging is None and self.blackboard.on_dock is None:
                return
            elif self.blackboard.robot_charging or self.blackboard.on_dock:
    ...
    def check_battery_cb(self):
        ...
        if not self.blackboard.fake and (self.blackboard.robot_charging or self.blackboard.on_dock):
            return
    ...
    def check_charging_cb(self):      
        ...
        if self.fake:
            if self.blackboard.robot_charging:
                rospy.logdebug("STILL FAKE CHARGING...")
                return TaskStatus.RUNNING
            else:
                rospy.logdebug("NOT FAKE CHARGING")
                print "RETURNING", TaskStatus.FAILURE
